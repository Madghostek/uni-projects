...
ImageInfo* __attribute__ ((noinline)) readBmp(const char* fileName)
{
    ...
    imginfo->line_bytes=(imginfo->line_bytes+3)&~3;
...
int __attribute__ ((noinline)) saveBmp(const char* fileName, ImageInfo *imageinfo)
{
...
void __attribute__ ((noinline)) freeImage(ImageInfo *imageinfo)
{
    if (imageinfo != NULL)
    {
