...
/* returns sum of user times of this group */
PUBLIC int do_getutime()
{
	int cumtime,proc_nr;
	message m1,m2;
	cumtime=0;
	...
	for(proc_nr = 0; proc_nr < NR_PROCS; ++proc_nr)
	{
		...
		{
			m1.m1_i1=proc_nr;
			_taskcall(SYSTASK, SYS_GETGROUP, &m1);
			if (m1.m1_i2==mm_in.m1_i1)
			{
				m2.m1_i1=proc_nr;
				_taskcall(SYSTASK, SYS_TIMES, &m2);
				cumtime+=m2.USER_TIME;
			}
		}
	}
	return cumtime;
...
PUBLIC int do_setquant()
{
	return _taskcall(SYSTASK, SYS_SETQUANT, &mm_in);
}