...
#define isGroupValid(group) ((group)>=0&&(group)<GROUP_COUNT)
 ...
 */
  ...
  current_quant=group_quants[rp->group];
  proc_ptr = rp;
  bill_ptr = rp;
  return;
  }
...
{
  struct proc* iter;
  iter = rdy_head[USER_Q];

  if (iter==NIL_PROC) return 0;

  while (iter->p_nextready!=NIL_PROC) iter = iter->p_nextready;

  if (iter!=rdy_tail[USER_Q]) return 1;
  return 0;
...
/*  if (verify_rdy())
  {
    printf("corrupted rdy\n");
  }*/

  /* if no, or only one process ready, just go to pick_proc */
  if (rdy_head[USER_Q] == NIL_PROC || rdy_head[USER_Q]->p_nextready==NIL_PROC)
  {
    pick_proc(); /* one process -> do nothing */
    return;
  }

  target_group = rdy_head[USER_Q]->group;
  /* Two or more user processes queued. */
  while (1) 
  {
    /* find process (the parent) with target group */
    target_group=(target_group+1)%GROUP_COUNT;
    iter = rdy_head[USER_Q];
    while (iter->p_nextready!=NIL_PROC && iter->p_nextready->group != target_group)
    {   
      iter=iter->p_nextready;
    }
    if (iter->p_nextready!=NIL_PROC) break;
  }

  target = iter->p_nextready;
  /* kick head to tail, move found process to head */
  /* 1. connect the gap where process was taken out from */
  iter->p_nextready=target->p_nextready;
  /* 2. attach the taken out process to be new head */
  target->p_nextready=rdy_head[USER_Q]->p_nextready;
  /* 43. make old head the tail*/
  rdy_head[USER_Q]->p_nextready=NIL_PROC;
  rdy_tail[USER_Q]->p_nextready=rdy_head[USER_Q];

  /* 5. update tail and head pointers */

  rdy_tail[USER_Q]=rdy_head[USER_Q];   /* tail is the process that was on head previously */
  rdy_head[USER_Q]=target;             /* head is the target */
...
PUBLIC int set_process_group(p, group)
struct proc* p;
int group;
{
  if (!isuserp(p) || !isGroupValid(group)) return 1;
