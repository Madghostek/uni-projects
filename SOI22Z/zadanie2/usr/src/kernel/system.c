...
FORWARD _PROTOTYPE( int do_syssetgroup, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_sysgetgroup, (message *m_ptr) );


PRIVATE int do_sysgetgroup(m_ptr)
register message *m_ptr;  /* pointer to request message */
{
	int res;
	struct proc* process;
	process=proc_addr(m_ptr->PROC1);
	if (isuserp(process))
	{
  		m_ptr->m1_i2=process->group;
  		return(OK);
	}
	/*"error, not userp\n");*/
	m_ptr->m1_i2=-1;
...
PRIVATE int do_syssetgroup(m_ptr)
register message *m_ptr;  /* pointer to request message */
{
  m_ptr->m1_i3=set_process_group(proc_addr(m_ptr->PROC1),m_ptr->m1_i2);
  return(OK); 
}

PRIVATE int do_syssetquant(m_ptr)
register message *m_ptr;  /* pointer to request message */
{
  if (m_ptr->m1_i2<1) m_ptr->m1_i2=1;
  group_quants[m_ptr->m1_i1]=m_ptr->m1_i2;
  return(OK); 
}
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
      ...
      case SYS_SETGROUP:  r = do_syssetgroup(&m);  break;
      case SYS_GETGROUP:  r = do_sysgetgroup(&m);  break;
      case SYS_SETQUANT:  r = do_syssetquant(&m);  break;
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  set_process_group(rpc,rpp->group);
