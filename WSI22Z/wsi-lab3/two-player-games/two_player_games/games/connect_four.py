...
class ConnectFourState(State):
    ...
    def get_moves(self) -> Iterable[ConnectFourMove]:
        return [i for i, column in enumerate(self.fields) if column[-1] is None]

    def get_current_player(self) -> Player:
        return self._current_player

    def make_move(self, move: ConnectFourMove) -> 'ConnectFourState':
        new_fields = [column[:] for column in self.fields]
    ...
    def __str__(self) -> str:
        ...
        text = '\n'.join(
            reversed([''.join(
                f'[{" " if field is None else field.char}]' for field in row) for row in transposed])
        )
